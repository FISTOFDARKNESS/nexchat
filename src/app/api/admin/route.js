...
import { triggerToUser, triggerToUsers, broadcastToAll, ONLINE_EXPR } from '@/lib/realtime';
...
const SENSITIVE_ADMIN_ACTIONS = ['set_role', 'grant_premium', 'revoke_premium', 'delete_message', 'delete_file', 'clear_2fa_lock'];

async function checkFullAdmin(userId) {
  if (!userId) return false;
  const users = await sql('SELECT role FROM "User" WHERE id = $1 LIMIT 1', [userId]);
  if (users.length === 0) return false;
...
async function kickUser(userId, reason = 'Sua conta foi desconectada por um administrador.') {
  if (!userId) return;
  await triggerToUser(userId, 'force_logout', { reason });
}
...
export async function GET(req) {
  try {
    ...
    if (action === 'stats') {
      const activeUsers = await sql(`SELECT COUNT(*) FROM "User" WHERE ${ONLINE_EXPR}`);
    ...
    if (action === 'users') {
      ...
      const users = await sql(
        `        SELECT u.id, u.username, u."customId", u.email, u.role, u."isGuest", ${ONLINE_EXPR} as "isOnline", u.gender, u.country,
                u."lastSeen", u."lastIp", u."premiumTier", u."premiumExpiresAt", u.verified, u."createdAt", u."twoFactorLockUntil",
...
export async function POST(req) {
  try {
    ...
    if (SENSITIVE_ADMIN_ACTIONS.includes(action)) {
      const isFullAdmin = await checkFullAdmin(adminUserId);
      if (!isFullAdmin) {
        return NextResponse.json({ error: 'Acesso negado: ação restrita a administradores' }, { status: 403 });
      }
    }

    if (['warn', 'ban', 'unban', 'set_role', 'kick', 'clear_2fa_lock'].includes(action) && !targetUserId) {
    ...
    if (action === 'ban') {
      ...
      await bumpSessionVersion(targetUserId).catch(() => {});
    ...
    if (action === 'set_role') {      const { role } = body; 
    ...
    if (action === 'delete_message') {
      ...
      if (table === 'direct') {
        ...
        directIds = [msgs[0].senderId, msgs[0].receiverId];
      ...
      if (table === 'direct' && directIds) {
        const ids = directIds.filter(Boolean);
        await triggerToUsers(ids, 'admin_msg_deleted', { messageId, table, fileId: deletedFileId });
        if (deletedFileId) await triggerToUsers(ids, 'media_deleted', { id: deletedFileId });
      } else if (groupId) {
        const members = await sql('SELECT "userId" FROM "GroupMember" WHERE "groupId" = $1', [groupId]);
    ...
    if (action === 'delete_file') {
      ...
      const files = await sql('SELECT "storageKey", "ownerId" FROM "File" WHERE id = $1', [fileId]);
      if (files.length === 0) {
      ...
      if (files[0].ownerId) {
        await triggerToUser(files[0].ownerId, 'media_deleted', { id: fileId });
      }
    ...
    if (action === 'clear_2fa_lock') {
      const updated = await sql(
        'UPDATE "User" SET "twoFactorLockUntil" = NULL, "twoFactorAttempts" = 0, "updatedAt" = now() WHERE id = $1 RETURNING *',
        [targetUserId]
      );
      if (updated.length === 0) {
        return NextResponse.json({ error: 'Usuário não encontrado' }, { status: 404 });
      }
      await logAdminAction(adminUserId, 'clear_2fa_lock', targetUserId, {});
      return NextResponse.json({ success: true, message: 'Bloqueio 2FA removido', user: updated[0] });
    }
    ...
    if (action === 'broadcast') {
      ...
      const sent = await broadcastToAll('global_announcement', {
        message: message.trim(),
        adminName: admin[0]?.username || 'Admin',
        createdAt: new Date().toISOString()
      });
      await logAdminAction(adminUserId, 'broadcast', null, { message: message.trim() });
      return NextResponse.json({ success: true, message: `Anúncio enviado para ${sent} usuários` });
    }
