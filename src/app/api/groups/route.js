  ...
  try {
    ...
    if (groupId) {
      ...
      const members = await sql(
        `SELECT u.id as "userId", u.username, u."customId", u."avatarUrl", ${ONLINE_EXPR} as "isOnline", u."premiumTier", u."premiumExpiresAt", u.verified, gm.role, gm."lastReadAt"
...
export async function POST(req) {
  try {
    ...
    if (action === 'create') {
      ...
      const cleanName = sanitizeContent(name.trim()).slice(0, 40);
      if (!cleanName) {
